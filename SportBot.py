...
def start(message):
    bot.send_message(message.chat.id,"Привет, {0.first_name}\nЭтот бот поможет тебе подобрать идеальную тренировку только для тебя".format(message.from_user))
    msg = bot.send_message(message.chat.id,"Введите вашу цель(похудение, или масса): ")
    bot.register_next_step_handler(msg, start_2)

def start_2(message):
     lock = str(message.text)
     if lock == "похудение" or lock == "Похудение" or lock == "Похудеть" or lock == "похудеть":
        ...
        bot.send_message(message.chat.id,"Неккоректный ввод, начните заново с команды /start")

def start_3(message):
    age = int(message.text)
    if age >= 60 or age <=16:
        bot.send_message(message.chat.id,"Перед тренировками необходима консультация у квалифицированного тренера\nhttps://taplink.cc/fito_nadya")
    else:
...
def start_4(message):
    ...
    if weight >= 90 or weight <= 40:
        ...
        contraindications = bot.send_message(message.chat.id,"Есть ли у вас противопоказания к занятию фитнесом(Да,Нет): ")
...
def start_5(message):
    ...
    if contraindications == "Да" or contraindications == "да":
        ...
        bot.send_message(message.chat.id,"https://disk.yandex.ru/d/7qyoil6QDNJTKA\nПриятных тренировок!")
        bot.send_message(message.chat.id,"А за любыми консультациями и дальнейшими тренировками, а так же питанием обращайтесь к\nhttps://taplink.cc/fito_nadya ")
...
def start_5_1(message):
    ...
    if contraindications == "Да" or contraindications == "да":
        ...
        bot.send_message(message.chat.id,"https://disk.yandex.ru/d/3-xtAs6UuZ2rGA\nПриятных тренировок!")
        bot.send_message(message.chat.id,"А за любыми консультациями и дальнейшими тренировками, а так же питанием обращайтесь к\nhttps://taplink.cc/fito_nadya ")
